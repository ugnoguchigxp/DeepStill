...
test("incremental slots preserve untouched sections and reject unknown targets", async () => {
	const { applyDraftUpdate, draftUpdateSchema, incrementalSchemaFor } =
		await import("../packages/research/deliverables");
	const paragraph = {
		text: "既存の仕組みと条件",
		kind: "finding" as const,
		citations: [{ sourceId: "s", firstLine: 1, lastLine: 2 }],
	};
	const previous = {
		...empty,
		sections: [
			{ title: "保持する説明", paragraphs: [paragraph] },
			{ title: "訂正対象", paragraphs: [paragraph] },
		],
	};
	const update = draftUpdateSchema.parse({
		replaceSections: {
			s0: null,
			s1: {
				reason: "新資料による訂正",
				value: {
					title: "訂正済み",
					paragraphs: [{ ...paragraph, text: "新しい条件" }],
				},
			},
		},
		appendSections: [],
	...
	const result = applyDraftUpdate(previous, update);
	expect(result.sections[0]).toEqual(previous.sections[0]);
	expect(result.sections[1].paragraphs[0].text).toBe("新しい条件");
	expect(previous.sections[1].title).toBe("訂正対象");
	const bad = {
		...update,
		replaceSections: {
			...update.replaceSections,
			s2: update.replaceSections.s1,
		},
	};
	expect(() => applyDraftUpdate(previous, bad)).toThrow(
		"INVALID_UPDATE_TARGET",
	);
	expect(
		incrementalSchemaFor(previous).safeParse({
			draft: null,
			update: bad,
			next: { kind: "finish", satisfied: true, reason: "done" },
		}).success,
	).toBe(false);
});
