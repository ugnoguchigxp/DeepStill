...
const sectionChange = z
	.object({ reason: text.max(400), value: draftSchema.shape.sections.element })
	.nullable();
const knowledgeChange = z
	.object({ reason: text.max(400), value: deliverableKnowledge })
	.nullable();
export const draftUpdateSchema = z.object({
	replaceSections: z.record(z.string().regex(/^s\d+$/), sectionChange),
	appendSections: draftSchema.shape.sections,
	replaceKnowledge: z.record(z.string().regex(/^k\d+$/), knowledgeChange),
	appendKnowledge: draftSchema.shape.knowledge,
	limitations: draftSchema.shape.limitations,
	openQuestions: draftSchema.shape.openQuestions,
});
export const incrementalStepSchema = z.object({
	draft: z.null(),
	update: draftUpdateSchema,
	next: actionSchema,
});
// Fixed named slots remove index arithmetic and duplicate replacement targets from generation.
export function incrementalSchemaFor(previous: Draft) {
	return incrementalStepSchema.extend({
		update: draftUpdateSchema.extend({
			replaceSections: z
				.object(
					Object.fromEntries(
						previous.sections.map((_, i) => [`s${i}`, sectionChange]),
					),
				)
				.strict(),
			replaceKnowledge: z
				.object(
					Object.fromEntries(
						previous.knowledge.map((_, i) => [`k${i}`, knowledgeChange]),
					),
				)
				.strict(),
		}),
...
export function applyDraftUpdate(
	previous: Draft,
	update: z.infer<typeof draftUpdateSchema>,
): Draft {
	function replace<T>(
		items: T[],
		changes: Record<string, { reason: string; value: T } | null>,
		additions: T[],
		prefix: string,
	) {
		const keys = new Set(items.map((_, i) => `${prefix}${i}`));
		for (const key of Object.keys(changes))
			if (!keys.has(key))
				throw Error(
					`INVALID_UPDATE_TARGET: ${key}; allowed=${[...keys].join(",")}; use append for new items`,
				);
		return [
			...items.map((item, i) => changes[`${prefix}${i}`]?.value ?? item),
			...additions,
		];
	}
	return draftSchema.parse({
		sections: replace(
			previous.sections,
			update.replaceSections,
			update.appendSections,
			"s",
		),
		knowledge: replace(
			previous.knowledge,
			update.replaceKnowledge,
