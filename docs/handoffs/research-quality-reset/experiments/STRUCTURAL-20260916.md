...
- mainでは既定無効とし、`DELIVERY_SECTION_UPDATES=1` のときだけ有効になる。候補コードは実験ブランチに保存し、回帰試験では明示的に有効化した。
...
判断は採用保留とする。固定再生では対象欠陥に明確な改善があったが、liveでは保持改善も探索完了率改善も確認できなかった。コードは再試験できるようフラグ配下に残すが、mainの新規live jobでは既定無効とする。削除・置換数を機械的に減らして保持率だけを上げる変更は、訂正すべき誤情報を固着させるため今回は行わない。
