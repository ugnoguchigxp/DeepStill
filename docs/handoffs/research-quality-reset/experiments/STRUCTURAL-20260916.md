...
- 候補コミット: `ff7b9c2`。保存ブランチ: `codex/experiment-structural-u3-20260916`。
- `bun run verify` 成功。38ファイル250テスト。既存lint warningのみ。
- `sectionId` は節見出しと同名出現順から決定し、`replace / add / delete` を検証してから完全なdraftへ合成する。出力にない節はコード側で保持する。未知ID、同じ節への二重操作、削除済み節の参照、合成後の上限超過を拒否する。
- 新規live jobでは既定で有効。`DELIVERY_SECTION_UPDATES=0` で切り戻せる。fixtureと既存jobの既定挙動は変えず、実験の回帰試験では明示的に有効化した。

同じ保存入力 `delivery:25:deliverable_step` の固定再生結果は次のとおり。どちらも新規Web探索はなく、引用とschemaは有効だった。

|条件|節 / 段落 / 文字|旧15段落の完全一致保持|消えた節|入力 / 出力|
|---|---|---:|---:|---:|
|全文再生成|6 / 14 / 1,979|0 / 15（0%）|1|21,027 / 4,175|
|U3節更新|5 / 18 / 3,444|12 / 15（80%）|0|21,724 / 2,760|

U3は1節を置換し1節を追加した。入力は697 tokens増えたが、出力は1,415 tokens減り、無関係な3節を完全保持した。固定再生は探索品質の証明ではないが、全文再生成による消失という対象欠陥には直接到達している。

live比較テーマは `フロンティアモデルではAGENTS.mdは不要に`。

|条件|実行ID|終了|query / URL / 資料|入力 / 出力|無効出力|節更新|
|---|---|---|---:|---:|---:|---:|
|基準 `e6e49af`|`af106cc4-e03b-49c9-b753-59f311946df3`|partial / unresolved_questions|2 / 2 / 2|64,899 / 7,476|1|0|
|U3候補 `ff7b9c2`|`38041bfa-2eba-4001-b9a3-218145c23eae`|partial / unresolved_questions|3 / 3 / 2|66,259 / 6,561|0|1|

両方が同じOpenAI公式記事を同一hashで読み、2件目の資料は異なった。候補の節更新は発動したが、2件目の資料が全3節に関係するとモデルが判断し、3節すべてを明示置換したため、liveでの完全一致保持率は0%のままだった。候補は無効出力を生まず、入力+1,360、出力-915、合計+445 tokensで、正常完了率は変わらなかった。資料差と探索経路差があるため、内容差やquery増加をU3の因果効果とは断定しない。

判断は限定採用とする。固定再生で対象欠陥に明確な改善があり、liveで重大な退行が見られなかったため、新規live jobの成果物更新契約として採用する。ただし探索完了率、内容点、liveでの保持改善は未達であり、U3を全体品質改善や80点達成とは扱わない。削除・置換数を機械的に減らして保持率だけを上げる変更は、訂正すべき誤情報を固着させるため今回は行わない。

## 累積使用量

この構造トライアルで今回新規に実行したprovider利用は次のとおり。予約値は加算していない。

- R6基準固定再生: 入力21,027 / 出力4,175。
- U3固定再生: 入力21,724 / 出力2,760。
- D2 live基準・候補: 入力172,665 / 出力28,132。
- U3 live基準・候補: 入力131,158 / 出力14,037。
- 合計: 入力346,574 / 出力49,104 tokens。
